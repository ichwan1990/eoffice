					...
					foreach ($row as $r => $data) { ?>
						<tr>
							...
							<td class="text-center">
								<div>
									<a href="<?= site_url('surat_keluar/edit/' . $data->id_surat_out) ?>" class="btn btn-xs btn-primary" title="Edit"><i class="fa fa-pen"></i></a>
									<a href="<?= site_url('surat_keluar/del/' . $data->id_surat_out) ?>" onclick="return confirm('Apakah Anda yakin?')" class="btn btn-xs btn-danger" title="Delete"><i class="fa fa-trash"></i></a>
								</div>
							</td>
						</tr>
					<?php } ?>
